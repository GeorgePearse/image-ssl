...
def create_test_image(size=(256, 256)):
    ...
    cv2.rectangle(img, (size[1] // 2, size[0] // 2),
                 (size[1] // 2 + 60, size[0] // 2 + 60), (0, 255, 0), -1)
    ...
    for method_idx, method in enumerate(methods):
        ...
        try:
            ...
            for order_idx in range(4):
                axes[method_idx, order_idx].text(0.5, 0.5, f"Error:\n{str(e)}",
                                                ha='center', va='center')
                axes[method_idx, order_idx].axis('off')
...
def demo_walk_statistics(image: np.ndarray, max_steps: int = 500):
    ...
    for name, strategy in strategies.items():
        ...
        colors = [step.value.mean() if isinstance(step.value, np.ndarray) else step.value
                 for step in path]
    ...
    for name, stat in stats.items():
        print(f"{name:<30} {stat['path_length']:<10} "
...
def main():
    ...
    try:
        ...
        cv2.imwrite('experiments/test_image.png',
